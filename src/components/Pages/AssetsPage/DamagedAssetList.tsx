...
import { useGetDemageGoodsStockQuery } from "@/redux/features/good_stock.features";
...
export default function DamagedAssetList() {
    const { data, isLoading } =
        useGetDemageGoodsStockQuery(undefined);

    const demages = data?.data || [];
    ...
    const totalPages = Math.ceil(
        demages.length / itemsPerPage
    );

    const paginatedDemages = useMemo(() => {
        const startIndex =
            (currentPage - 1) * itemsPerPage;

        return demages.slice(
            startIndex,
